...
if (action === 'cats') {
  const catOwnerQuery = 'SELECT cats.name AS cat_name, owner_id, owners.name AS owner_name FROM cats INNER JOIN owners ON cats.owner_id = owners.id;';

  client.query(catOwnerQuery, (error, result) => {
    if (error) {
      handleQueryError(error);
      return;
    }
    ...
    const catsNowners = result.rows;
    console.log(`my catsNowners are`, result.rows)
    catsNowners.forEach((cat, index) => {
   
        console.log(`${index + 1}. ${cat.cat_name}: Owner: ${cat.owner_name}`); // 1. Fluffy: Owner: Jim
      });
...
if (action === 'owners') {
  const catOwnerQuery = 'SELECT cats.name AS cat_name, owner_id, owners.name AS owner_name FROM cats INNER JOIN owners ON cats.owner_id = owners.id ORDER BY owner_id ASC ';
  ...
  client.query(catOwnerQuery, (error, result) => {
    ...
    // console.log(`my catsNowners are`, catsNowners)

    //first cat owner
    console.log(`1. ${catsNowners[0].owner_name} \n\t - Cats: \n\t\t`)
    for (let i=0; i< catsNowners.length; i+=1){
    
      if (`${catsNowners[i+1].owner_id}` === `${catsNowners[i].owner_id}` ){
 
        console.log(`\n\t\t ${catsNowners[i].cat_name}`)
          
      } else {
        console.log( `${catsNowners[i].owner_id+1} .${catsNowners[i+1].owner_name} \n\t - Cats: \n\t\t -${catsNowners[i+1].cat_name}  `)
      }
    }
